...
// Updated utility function to determine device type based on rule groups and additional log properties
const getDeviceType = (log) => {
    ...
    const agentName = (log.agent?.name || '').toLowerCase();
    const ruleDesc = (log.rule?.description || '').toLowerCase();

    if (groups.includes('firewall') || groups.includes('fortigate') || groups.includes('mikrotik') ||
        agentName.includes('firewall') || ruleDesc.includes('firewall') || ruleDesc.includes('vpn') ||
        groups.some(g => g.match(/forti|mikro|fw/i)) || ruleDesc.match(/forti|mikro|fw/i)) {
        return 'Firewall';
    } else if (groups.includes('windows')) {
        return 'Windows';
    } else if (groups.includes('syslog') && !groups.includes('firewall') && !groups.includes('fortigate') && !groups.includes('mikrotik')) {
        return 'Linux';
...
// Updated utility function to determine auth result with expanded checks for firewall failures
const getAuthResult = (log) => {
    ...
    const groups = log.rule.groups.map(g => g.toLowerCase());
    const ruleDesc = (log.rule?.description || '').toLowerCase();
    const action = (log.data?.action || '').toLowerCase();

    if (groups.includes('authentication_success') || ruleDesc.includes('successful login') || action.includes('login success') ||
        ruleDesc.match(/success|ok|logged in/i) || action.match(/success|ok/i)) {
        return 'Success';
    } else if (groups.includes('authentication_failed') || ruleDesc.includes('failed login') || ruleDesc.includes('authentication failure') ||
               action.includes('login failed') || action.includes('access denied') ||
               ruleDesc.match(/failed|failure|denied|invalid/i) || action.match(/failed|failure|denied|invalid/i)) {
        return 'Failed';
    } else {
        return 'Unknown';
    }
...
const Session = () => {
    ...
    useEffect(() => {
        if (stats) {
            // No console.log here for production
        }
    }, [stats]);
    ...
    const fetchSessionLogs = async (
        ...
        try {
            ...
            if (response) {
                ...
                console.log('Device Type Counts:', response.stats?.byDeviceType);
                console.log('Firewall counts:', {
                    byDeviceType: response.stats?.byDeviceType?.firewall,
                    byAuthAndDevice: response.stats?.byAuthAndDevice?.firewall
                });
                ...
                const normalizedLogs = response.logs.map(log => ({
                    ...
                    rule: {
                        ...log.rule,
                        groups: Array.isArray(log.rule.groups) ? log.rule.groups : (typeof log.rule.groups === 'string' ? [log.rule.groups] : []),
                        description: log.rule.description || ''
                    },
                    agent: { name: log.agent.name || '' },
                    data: { action: log.data.action || '' }
                }));
    ...
    // Handle device filter change with overlap exclusion
    // Handle device filter change - remove client-side filtering
const handleDeviceFilterChange = (event) => {
    const newValue = event.target.value;
    setDeviceFilterValue(newValue);
    // Just fetch with the new filter, don't manually filter client-side
    ...
    const getDeviceDistributionChartOption = () => {
        ...
        if (pieData.length === 0) {
            return {
                title: {
                    text: 'Operating System Distribution (No Data)',
                    left: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
                        fontFamily: theme.typography.fontFamily,
    ...
    const getAuthResultsDistributionChartOption = () => {
        ...
        if (pieData.length === 0) {
            return {
                title: {
                    text: 'Authentication Status (No Data)',
                    left: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
                        fontFamily: theme.typography.fontFamily,
...
const renderDeviceAuthCard = (title, data = {}, deviceType, icon) => {
    console.log(`🎨 Rendering ${title} card - data:`, data);
    ...
    const getNumberFontSize = (number) => {
        const numLength = number.toString().length;
        if (numLength > 8) return '1.25rem';    // For very large numbers (100,000,000+)
        if (numLength > 6) return '1.5rem';     // For large numbers (1,000,000+)
        if (numLength > 4) return '1.75rem';    // For medium numbers (10,000+)
        return '2.125rem'; // default h4 size for small numbers
    };

    // Compact number formatting for very large numbers
            ...
            >
                <CardContent sx={{ p: 3 }}>
                    <Box sx={{
                        ...
                        <Tooltip title={`Total: ${totalCount.toLocaleString()}`}>
                            <Chip
                                label={`Total: ${formatCompactNumber(totalCount)}`}
                                size="small"
                                sx={{
                                    fontWeight: 600,
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={6}>
                            ...
                            >
                                ...
                                <Tooltip title={successCount.toLocaleString()} arrow>
                                    <Typography 
                                        sx={{ 
                                            fontWeight: 700, 
                                            color: COLOR_PALETTE[0],
                                            fontSize: getNumberFontSize(successCount),
                                            lineHeight: 1.2,
                                            wordBreak: 'break-all',
                                            minHeight: '2.5rem',
                                            display: 'flex',
                                    ...
                                    >
                                        {formatCompactNumber(successCount)}
                                    </Typography>
                                </Tooltip>
                                <Typography
                                    variant="caption"
                        ...
                        <Grid item xs={6}>
                            ...
                            >
                                ...
                                <Tooltip title={failedCount.toLocaleString()} arrow>
                                    <Typography 
                                        sx={{ 
                                            fontWeight: 700, 
                                            color: COLOR_PALETTE[1],
                                            fontSize: getNumberFontSize(failedCount),
                                            lineHeight: 1.2,
                                            wordBreak: 'break-all',
                                            minHeight: '2.5rem',
                                            display: 'flex',
        ...
        <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Box>
                    ...
                    >
                        Analyze user session activities, authentication events, and access patterns to identify suspicious behavior.
                    </Typography>
                </Box>
                        ...
                        <Box sx={{
                            ...
                            <Box sx={{ display: 'flex', gap: 2 }}>
                                <FormControl size="small" sx={{ minWidth: 180 }}>
                                    <InputLabel id="auth-result-filter-label">Authentication Status</InputLabel>
                                    <Select
            ...
            {/* change by raman making charts full screen  */}
