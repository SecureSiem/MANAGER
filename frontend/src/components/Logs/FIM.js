...
const FimDetailsView = ({ data, onClose, onViewFullDetails }) => {
  ...
  return (
    // added the full screen button and layout by raman
    <Dialog
  open={true}
  onClose={onClose}
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          ...
          >
            Track changes to critical files and directories in real-time to detect unauthorized modifications and maintain system integrity.
          </Typography>
        </Box>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }} ref={dashboardRef}>
        ...
        <Grid item xs={12} sm={4}>
          <Card
            ...
            sx={{
              borderRadius: 2,
              background: 'linear-gradient(135deg, rgba(255, 94, 87, 0.85), rgba(255, 0, 77, 0.65))',
              position: 'relative',
              overflow: 'hidden',
              boxShadow: '0 4px 20px rgba(0, 0, 0, 0.1)',
              transition: 'transform 0.3s ease',
