...
const addCoverPage = (pdf, timeRange, generatedDate) => {
  ...
  img.src = logo;
  img.onload = () => {
    pdf.addImage(img, 'PNG', centerX - 30, 30, 60, 30); // Logo image
    finishCoverText();
  };

  const finishCoverText = () => {
    // Add client logo placeholder
    pdf.setFontSize(18);
    const clientLogoBase64 = localStorage.getItem('clientLogo');
    if (clientLogoBase64) {
      pdf.addImage(clientLogoBase64, 'PNG', centerX - 30, 85, 60, 25);
    } else {
      pdf.setFontSize(18);
