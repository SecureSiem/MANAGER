...
const Tsc = () => {
  ...
  setLogs(prevLogs => prevLogs.filter(log => log.id !== deletedId));
    ...
    const getTimelineChartOption = () => {
        ...
        // If no data, return a blank template chart
        if (Object.keys(timelineData).length === 0) {
            return {
                title: {
                    text: 'TSC Events Timeline',
                    subtext: 'No data available for the selected time range',
                    left: 'center',
                    top: 'center',
                    textStyle: {
        ...
        if (Object.keys(ruleLevelData).length === 0) {
            return {
                title: {
                    ...
                    subtext: 'No severity data available',
                    left: 'center',
                    top: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
        ...
        if (agentData.length === 0) {
            return {
                title: {
                    ...
                    subtext: 'No agent data available',
                    left: 'center',
                    top: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
        ...
        if (descriptionData.length === 0) {
            return {
                title: {
                    ...
                    subtext: 'No rule description data available',
                    left: 'center',
                    top: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
        ...
        if (groupsData.length === 0) {
            return {
                title: {
                    ...
                    subtext: 'No rule group data available',
                    left: 'center',
                    top: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
        ...
        if (controlsData.length === 0) {
            return {
                title: {
                    ...
                    subtext: 'No TSC control data available',
                    left: 'center',
                    top: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
                            ...
                            return (
                                ...
                                >
                                    <Chip
                                        ...
                                        size="small"
                                       sx={{
  bgcolor: theme.palette.mode === 'dark' ? 'rgba(98, 54, 255, 0)' : '#eae6ff',
  color: '#6236FF',
  fontWeight: 500,
  fontSize: '0.7rem',
  height: 20,
  ...
  borderWidth: 1.5,
  borderStyle: 'solid',
  boxShadow: '0 0 5px rgba(98, 54, 255, 0.7)',
}}
