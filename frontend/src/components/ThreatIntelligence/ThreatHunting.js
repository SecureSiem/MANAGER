...
const ThreatHunting = () => {
    ...
    const processChartData = () => {
        ...
        // Timeline data with better error handling
        const timeData = {};
        if (stats.timeDistribution && Array.isArray(stats.timeDistribution) && stats.timeDistribution.length > 0) {
            stats.timeDistribution.forEach(item => {
                if (item && item.date && item.count !== undefined) {
                    try {
                        ...
                        console.warn("Error formatting date for timeline:", e);
                    }
                }
            });
        }
        ...
        const safeArrayExtract = (data, fallback = []) => {
        ...
        return {
            ...
            byActionData: safeArrayExtract(stats.byAction),
            byDirectionData: safeArrayExtract(stats.byDirection),
            byMessageData: safeArrayExtract(stats.byMessage),
            byAppListData: safeArrayExtract(stats.byAppList),
            byAppRiskData: safeArrayExtract(stats.byAppRisk),
            byLevelData: safeArrayExtract(stats.byLevel),
            bySrcCountryData: safeArrayExtract(stats.bySrcCountry),
            byDstCountryData: safeArrayExtract(stats.byDstCountry)
        };
    };
    ...
    // Actions Pie Chart
    // Actions Pie Chart - Updated with conditional legend display
    const getActionsPieChartOption = (isFullscreen = false) => {
        const chartData = processChartData();
        ...
        if (actionData.length === 0) {
            return {
                title: {
                    text: 'Activity Distribution (No Data)',
                    left: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
                        fontFamily: theme.typography.fontFamily,
        ...
        return {
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    <div style="display: flex; align-items: center; margin-bottom: 4px;">
                        <div style="width: 12px; height: 12px; background-color: ${params.color}; margin-right: 8px; border-radius: 2px;"></div>
                        <strong style="font-size: ${isFullscreen ? '14px' : '13px'};">${params.name}</strong>
                    </div>
                    <div style="margin-left: 20px;">
                        <div>Count: <span style="color:${params.color};font-weight:bold;">${params.value.toLocaleString()}</span></div>
                        <div>Percentage: <span style="font-weight:bold;">${params.percent.toFixed(1)}%</span></div>
                    </div>
                </div>`;
                }
            },
            ...
            legend: isFullscreen ? {
                ...
                formatter: function (name) {
                    const item = actionData.find(a => a.action === name);
                    if (!item) return name;
                    const percentage = ((item.count / total) * 100).toFixed(1);
                    return `${name}: ${item.count.toLocaleString()} (${percentage}%)`;
                },
                itemGap: 15,
                itemWidth: 14,
                itemHeight: 14
            } : {
                show: false  // Hide legend in minimal view
            },
            series: [{
                type: 'pie',
                radius: isFullscreen ? ['35%', '65%'] : ['40%', '70%'],
                ...
                label: {
                    show: isFullscreen,
                    position: isFullscreen ? 'outside' : 'center',
                    fontSize: isFullscreen ? 12 : 14,
                    fontWeight: isFullscreen ? 'normal' : 'bold',
                    color: theme.palette.text.primary,
                    formatter: function (params) {
                        if (isFullscreen) {
                            return `${params.name}\n${params.percent.toFixed(1)}%`;
                        } else {
                            // In minimal view, show total count in center
                ...
                labelLine: {
                    show: isFullscreen,
                    length: isFullscreen ? 15 : 0,
                    length2: isFullscreen ? 8 : 0,
                    lineStyle: {
                        color: theme.palette.text.secondary,
                        width: 1
                    }
            ...
            animation: true,
            animationDuration: isFullscreen ? 1000 : 800,
    ...
    const getMessageChartOption = () => {
        ...
        if (messageData.length === 0) {
            return {
                title: {
                    text: 'Top Current Messages (No Data)',
                    left: 'center',
                    textStyle: {
                        color: theme.palette.mode === 'dark' ? '#fff' : '#333',
                        fontFamily: theme.typography.fontFamily,
    ...
    //added the full screen functionality by raman 
    ...
    const ThreatHuntingDetailsView = ({ data, onClose, onViewFullDetails }) => {
        ...
        return (
            ...
            // added by raman
            <Dialog
                open={true}               // keep as it is - or pass dynamic open state
                onClose={onClose}
                fullScreen={fullscreen}   // toggle fullscreen mode based on state
                maxWidth={fullscreen ? false : "lg"}  // no max width in fullscreen
                fullWidth={!fullscreen}            // fill width only if not fullscreen
                scroll="paper"
                PaperProps={{
                    sx: fullscreen
                        ? {
    ...
    return (
        <Box>
            ...
            {loading && !logs.length && !stats ? (
                ...
                >
                    <Typography variant="subtitle1" gutterBottom>
                        Failed to load threat hunting data
                    </Typography>
                    <Typography variant="body2">
                        {error}
                    </Typography>
                ...
                >
                    <Box sx={{ textAlign: 'center', p: 4 }}>
                        <HuntingIcon sx={{ fontSize: 80, mb: 2, color: 'text.secondary', opacity: 0.3 }} />
                        <Typography variant="h5" color="text.secondary" gutterBottom>
                            No Threat Events Found
                        </Typography>
                        <Typography variant="body1" color="text.secondary" sx={{ mb: 3, maxWidth: 400 }}>
                            We couldn't find any threat hunting events for the selected time range.
                            Try adjusting your time range or check back later for new threat intelligence.
                        </Typography>
                        <Button
                            variant="outlined"
                            startIcon={<RefreshIcon />}
                                ...
                                <Grid container spacing={3} sx={{ mb: 3 }}>
                                    <Grid item xs={12} sm={6} md={3}>
                                        <Card elevation={3}
                                            sx={{
                                                borderRadius: 2,
                                                background: 'linear-gradient(135deg, rgba(255, 0, 102, 0.85), rgba(255, 94, 98, 0.7))',
                                                position: 'relative',
                                                overflow: 'hidden',
                                    ...
                                    <Grid item xs={12} sm={6} md={3}>
                                        <Card elevation={3} sx={{
                                            borderRadius: 2,
                                            background: 'linear-gradient(135deg, rgba(255, 45, 45, 0.85), rgba(139, 0, 0, 0.7))',
                                            position: 'relative',
                                            overflow: 'hidden',
                                            boxShadow: '0 4px 20px rgba(255, 0, 76, 0.2)',
                                            transition: 'transform 0.3s ease, box-shadow 0.3s ease',
                                            backdropFilter: 'blur(12px)',
                                            WebkitBackdropFilter: 'blur(12px)',
                                            ...
                                            '&:hover': {
                                                ...
                                                boxShadow: '0 6px 30px rgba(255, 0, 76, 0.35)',
                                            }
                                        }}
                                    ...
                                    <Grid item xs={12} sm={6} md={3}>
                                        <Card elevation={3} sx={{
                                            borderRadius: 2,
                                            background: 'linear-gradient(135deg, rgba(3, 169, 244, 0.85), rgba(0, 105, 192, 0.65))',
                                            position: 'relative',
                                            overflow: 'hidden',
                                            boxShadow: '0 4px 20px rgba(0, 145, 255, 0.2)',
                                            transition: 'transform 0.3s ease, box-shadow 0.3s ease',
                                            backdropFilter: 'blur(12px)',
                                            WebkitBackdropFilter: 'blur(12px)',
                                            ...
                                            '&:hover': {
                                                transform: 'translateY(-5px)',
                                                boxShadow: '0 6px 30px rgba(0, 145, 255, 0.35)',
                                            }
                                        }}
                                        // change by raman
                                        >
                                        ...
                                        >
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                                <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center' }}>
                                                    ...
                                                    <Box component="span" sx={{ ml: 1 }}>Activity Distribution</Box>
                                                </Typography>
                                                <Tooltip title="View Fullscreen">
                                                    <IconButton
                                                        size="small"
                                                        onClick={() => openFullscreen(getActionsPieChartOption(true), 'Activity Distribution')} // Pass true for fullscreen
                                                        className="fullscreen-icon"
                                                        sx={{
                                                            bgcolor: theme.palette.background.paper,
                                                            boxShadow: 1,
                                            ...
                                            <Box sx={{ height: 300 }}>
                                                <ReactECharts
                                                    option={getActionsPieChartOption(false)} // Pass false for minimal view
                                                    style={{ height: '100%', width: '100%' }}
                                                    opts={{ renderer: 'canvas' }}
                                                    theme={theme.palette.mode === 'dark' ? 'dark' : ''}
                                                    notMerge={true}
                                        ...
                                        >
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                                <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center' }}>
                                                    ...
                                                    <Box component="span" sx={{ ml: 1 }}>Top Source Countires</Box>
                                                </Typography>
                                                <Tooltip title="View Fullscreen">
                                                    <IconButton
                                                        size="small"
                                                        onClick={() => openFullscreen(getAppListChartOption(), 'Top Source Countries')}
                                                        className="fullscreen-icon"
                                                        sx={{
                                                            bgcolor: theme.palette.background.paper,
                                                            boxShadow: 1,
                                        ...
                                        >
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                                <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center' }}>
                                                    ...
                                                    <Box component="span" sx={{ ml: 1 }}>Top Current Messages</Box>
                                                </Typography>
                                                <Tooltip title="View Fullscreen">
                                                    <IconButton
                                                        size="small"
                                                        onClick={() => openFullscreen(getMessageChartOption(), 'Top Current Messages')}
                                                        className="fullscreen-icon"
                                                        sx={{
                                                            bgcolor: theme.palette.background.paper,
                                                            boxShadow: 1,
                                    ...
                                    <Grid item xs={12} md={6}>
                                        ...
                                        >
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                                <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center' }}>
                                                    <AppsIcon sx={{ mr: 1, color: '#4ECDC4' }} />
                                                    <Box component="span" sx={{ ml: 1 }}>Top Source Countries</Box>
                                                </Typography>
                                                <Tooltip title="View Fullscreen">
                                                    <IconButton
                                                        size="small"
                                                        onClick={() => openFullscreen(getSourceCountriesChartOption(), 'Top Source Countries')}
                                                        className="fullscreen-icon"
                                                        sx={{
                                                            bgcolor: theme.palette.background.paper,
                                                            boxShadow: 1,
                                    ...
                                    <Grid item xs={12} md={6}>
                                        ...
                                        >
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                                <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center' }}>
                                                    <MessageIcon sx={{ mr: 1, color: '#9C27B0' }} />
                                                    <Box component="span" sx={{ ml: 1 }}>Top Destination Countries</Box>
                                                </Typography>
                                                <Tooltip title="View Fullscreen">
                                                    <IconButton
                                                        size="small"
                                                        onClick={() => openFullscreen(getDestinationCountriesChartOption(), 'Top Destination Countries')}
                                                        className="fullscreen-icon"
                                                        sx={{
                                                            bgcolor: theme.palette.background.paper,
                                                            boxShadow: 1,
            ...
            >
                ...
                <DialogContent dividers sx={{ p: 0, height: 'calc(100vh - 120px)' }}>
