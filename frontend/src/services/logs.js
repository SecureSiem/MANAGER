...
// Get Tor Browser logs
export const getTorBrowserLogs = async (params = {}) => {
  try {
    console.log('Fetching Tor Browser logs with params:', params);

    const response = await api.get('/logs/tor-browser', { params });

    if (!response.data || !response.data.logs) {
      throw new Error('Invalid response format from server');
    }

    console.log(`Received ${response.data.logs.length} Tor Browser logs out of ${response.data.pagination?.total}`);
    ...
    if (error.response) {
      ...
      throw new Error(error.response.data.message || 'Failed to fetch Tor Browser logs');
    }
...
// added by raman to check delete functionality for the log 
...
// Get endpoint-specific analytics and logs (combined endpoint)
export const getEndpointAnalytics = async (endpoint, timeRange = '7d', page = 1, limit = 50, search = '') => {
  try {
    ...
    // Response structure: { analytics: {...}, logs: [...], pagination: {...} }
    return response.data;
  } catch (error) {
...
export const startLogExport = async (timeRange) => {
  try {
    console.log('Starting log export for timeRange:', timeRange);

    const response = await api.post('/logs/export/start', { timeRange });

    if (!response.data || !response.data.jobId) {
      throw new Error('Invalid response from export start');
    }

    console.log('Export job started:', response.data.jobId);
    ...
    if (error.response) {
      console.error('Error details:', error.response.data);
      throw new Error(error.response.data.message || 'Failed to start export');
    }
...
export const getExportStatus = async (jobId) => {
  try {
    const response = await api.get(`/logs/export/status/${jobId}`);

    if (!response.data) {
      throw new Error('Invalid response from export status check');
    }
    ...
    if (error.response) {
      if (error.response.status === 404) {
        throw new Error('Export job not found or expired');
      }
      throw new Error(error.response.data.message || 'Failed to check export status');
    }
