    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        try {
          ...
          // Transform the log (now async and includes false positive checking)
          const transformedLog = await transformLog(rawLog);
          ...
          try {
            ...
            console.log(`Successfully indexed log with ID ${transformedLog.id} to ${index}${transformedLog.is_false_positive ? ' (false positive)' : ''}`);
          } catch (indexError) {
            ...
            try {
              ...
              if (indexError.message.includes('mapper_parsing_exception')) {
                ...
                console.log(`Successfully indexed sanitized log with ID ${safeLog.id} to ${index}${safeLog.is_false_positive ? ' (false positive)' : ''}`);
              }
            } catch (fallbackError) {
