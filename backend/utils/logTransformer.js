...
const { checkFalsePositiveRules } = require('../services/falsePositiveProcessor');
// Transform a log from raw format to a structured format
const transformLog = async (rawLog) => {
  try {
    ...
    try {
      const fpCheck = await checkFalsePositiveRules(structuredLog);
      structuredLog.is_false_positive = fpCheck.isFalsePositive;

      if (fpCheck.isFalsePositive && fpCheck.matchedRule) {
        structuredLog.false_positive_rules = [fpCheck.matchedRule];
      } else {
        structuredLog.is_false_positive = false;
      }
    } catch (error) {
    ...
    return {
      ...
      'id': `error-${Date.now()}`,
      'is_false_positive': false, // Add this to error logs too
...
const createEmptyLogStructure = () => {
  return {
    '@timestamp': new Date().toISOString(),
    'id': `unknown-${Date.now()}`,
    'is_false_positive': false, // Add this line
    'location': null,
    'agent': {
      'name': 'unknown',
      'id': 'unknown',
  ...
  if (log.raw_log) {
    ...
    if (typeof log.raw_log === 'string') {
      try {
        ...
        console.warn('Could not parse raw_log for location:', e.message);
      }
    }
  }
